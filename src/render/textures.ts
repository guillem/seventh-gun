...
// HUSK — drowned-grey meat with the ribs showing through. Cold slate base,
// four flat bone-grey rib bands (they ring the lathe torso once each), a
// dark channel where the sternum splits and the spine runs, and a few big
// bruise blotches. No plating, no all-over dither: the mesh carries the
// shape, the skin only carries value.
function skinHusk(): HTMLCanvasElement {
  const { c, g } = canvas(64);
  const rng = makeRng('skin-husk').float;
  g.fillStyle = '#22262b'; // cold slate
  g.fillRect(0, 0, 64, 64);
  // bruises under the skin: few and big
  for (let i = 0; i < 10; i++) {
    g.fillStyle = `rgba(${8 + rng() * 10 | 0},${10 + rng() * 10 | 0},${18 + rng() * 14 | 0},0.55)`;
    g.beginPath(); g.arc(rng() * 64, rng() * 64, 4 + rng() * 7, 0, Math.PI * 2); g.fill();
  }
  // rib bands: a shadow groove with a pale ridge above it. Kept quiet so a
  // limb wrapped in this reads as segmented, not striped.
  for (let y = 6; y < 64; y += 16) {
    g.fillStyle = 'rgba(8,9,12,0.7)';
    g.fillRect(0, y + 3, 64, 3);
    g.fillStyle = 'rgba(122,120,112,0.42)';
    g.fillRect(0, y, 64, 3);
    g.fillStyle = 'rgba(180,176,166,0.3)';
    g.fillRect(0, y, 64, 1);
  }
  // the sternum split (front) and the spine channel (back) cut the bands
  for (const x of [16, 48]) {
    g.fillStyle = '#101317';
    g.fillRect(x - 2, 0, 4, 64);
    g.fillStyle = 'rgba(70,76,84,0.45)';
    g.fillRect(x + 2, 0, 1, 64);
  }
  noise(g, 64, rng, 250, 0.1);
  return c;
}

// SLAB — sooty iron hide. Near-black umber base, two riveted plate seams
// that ring the lathe body, one vertical seam, and a few big scorch blooms
// where its own heat has cooked the plate. The mesh carries the mass; the
// skin only says "bolted metal".
function skinSlab(): HTMLCanvasElement {
  const { c, g } = canvas(64);
  const rng = makeRng('skin-slab').float;
  g.fillStyle = '#2a2018'; // sooty umber
  g.fillRect(0, 0, 64, 64);
  // scorch blooms: few and big, warm rust under the soot
  for (let i = 0; i < 5; i++) {
    g.fillStyle = `rgba(${110 + rng() * 40 | 0},${50 + rng() * 20 | 0},${14 + rng() * 10 | 0},0.2)`;
    g.beginPath(); g.arc(rng() * 64, rng() * 64, 5 + rng() * 8, 0, Math.PI * 2); g.fill();
  }
  // plate seams: a dark groove with a lit bevel above, period 32 so they tile
  for (const y of [10, 42]) {
    g.fillStyle = 'rgba(8,6,4,0.8)';
    g.fillRect(0, y, 64, 3);
    g.fillStyle = 'rgba(150,140,120,0.35)';
    g.fillRect(0, y - 1, 64, 1);
    // rivet heads along the seam: small, or they smear into blotches on the
    // big lathe body
    for (let x = 8; x < 64; x += 16) {
      g.fillStyle = 'rgba(8,6,4,0.8)';
      g.beginPath(); g.arc(x + 0.5, y + 5.5, 1.9, 0, Math.PI * 2); g.fill();
      g.fillStyle = 'rgba(140,132,118,0.7)';
      g.beginPath(); g.arc(x, y + 5, 1.4, 0, Math.PI * 2); g.fill();
    }
  }
  // one vertical seam to break the banding
  g.fillStyle = 'rgba(8,6,4,0.8)';
  g.fillRect(31, 0, 3, 64);
...
// HIEROPHANT — void-purple vestment. Near-black violet base, vertical pleats
// that run the length of the lathe robe, a faint hem band, and a few deep
// void pools in the folds. No inlay, no plates: the mask and mitre carry the
// priest read, the robe only has to hang.
function skinHierophant(): HTMLCanvasElement {
  const { c, g } = canvas(64);
  const rng = makeRng('skin-hier').float;
  g.fillStyle = '#241c2e'; // void violet
  g.fillRect(0, 0, 64, 64);
  // void pools in the folds: few and big
  for (let i = 0; i < 6; i++) {
    g.fillStyle = `rgba(${40 + rng() * 30 | 0},${16 + rng() * 12 | 0},${60 + rng() * 40 | 0},0.35)`;
    g.beginPath(); g.arc(rng() * 64, rng() * 64, 5 + rng() * 8, 0, Math.PI * 2); g.fill();
  }
  // pleats: a shadow groove and a lit ridge every 16px, period divides 64
  for (let x = 4; x < 64; x += 16) {
    g.fillStyle = 'rgba(6,4,10,0.75)';
    g.fillRect(x, 0, 3, 64);
    g.fillStyle = 'rgba(120,96,150,0.32)';
    g.fillRect(x + 3, 0, 1, 64);
  }
  // hem band low on the tile
  g.fillStyle = 'rgba(6,4,10,0.6)';
  g.fillRect(0, 56, 64, 2);
  g.fillStyle = 'rgba(150,120,90,0.3)';
  g.fillRect(0, 58, 64, 1);
  noise(g, 64, rng, 220, 0.08);
  return c;
}

// FIEND — dark crimson hide. Near-black red base, a few big char patches,
// and two staggered rows of scale ridges that ring the lathe chest. No
// ember in the skin: the heat lives on the mesh (eyes, tail tip) so the
// body stays a dark mass.
function skinFiend(): HTMLCanvasElement {
  const { c, g } = canvas(64);
  const rng = makeRng('skin-fiend').float;
  g.fillStyle = '#3a0f16'; // dark crimson hide
  g.fillRect(0, 0, 64, 64);
  // char patches: few and big
  for (let i = 0; i < 6; i++) {
    g.fillStyle = `rgba(${10 + rng() * 10 | 0},${4 + rng() * 4 | 0},${4 + rng() * 6 | 0},0.5)`;
    g.beginPath(); g.arc(rng() * 64, rng() * 64, 5 + rng() * 8, 0, Math.PI * 2); g.fill();
  }
  // scale ridges: short dark arcs with a lit crown, two rows, period 32
  for (const [y, off] of [[14, 0], [46, 8]]) {
    for (let x = off; x < 64 + off; x += 16) {
      g.fillStyle = 'rgba(8,2,4,0.75)';
      g.beginPath(); g.arc(x, y + 4, 7, Math.PI, 0); g.fill();
      g.fillStyle = 'rgba(120,50,50,0.5)';
      g.beginPath(); g.arc(x, y + 5, 5.5, Math.PI, 0); g.fill();
      g.fillStyle = 'rgba(190,110,90,0.2)';
      g.beginPath(); g.arc(x, y + 5, 5.5, Math.PI * 1.15, Math.PI * 1.85); g.fill();
    }
  }
  noise(g, 64, rng, 240, 0.09);
  return c;
}
