...
describe('protocol guards', () => {
  it('accepts valid client frames', () => {
    ...
    expect(isClientMessage({
      v: 2, t: 'input', spawnCount: 1, seq: 1,
  ...
  it('requires a positive life epoch on each input batch', () => {
    ...
    expect(isClientMessage({ v: 2, t: 'input', seq: 1, inputs: [frame] })).toBe(false);
    expect(isClientMessage({ v: 2, t: 'input', spawnCount: 0, seq: 1, inputs: [frame] })).toBe(false);
  });
  ...
  it('rejects non-finite numbers and bad switchGun / seq', () => {
    ...
    expect(isClientMessage({ v: 2, t: 'input', spawnCount: 1, seq: 1, inputs: [{ ...base, yaw: NaN }] })).toBe(false);
    expect(isClientMessage({ v: 2, t: 'input', spawnCount: 1, seq: 1, inputs: [{ ...base, pitch: Infinity }] })).toBe(false);
    expect(isClientMessage({ v: 2, t: 'input', spawnCount: 1, seq: 1, inputs: [{ ...base, moveX: Number.NaN }] })).toBe(false);
    expect(isClientMessage({ v: 2, t: 'input', spawnCount: 1, seq: -1, inputs: [base] })).toBe(false);
    expect(isClientMessage({ v: 2, t: 'input', spawnCount: 1, seq: 1.5, inputs: [base] })).toBe(false);
    expect(isClientMessage({ v: 2, t: 'input', spawnCount: 1, seq: 1, inputs: [{ ...base, switchGun: 8 }] })).toBe(false);
    expect(isClientMessage({ v: 2, t: 'input', spawnCount: 1, seq: 1, inputs: [{ ...base, switchGun: 3 }] })).toBe(true);
