...
describe('ArenaClient', () => {
  it('deduplicates only the matching predicted shot identity', () => {
    const client = new ArenaClient() as unknown as {
      id: number;
      predictedShotSeqs: Set<number>;
      shouldIgnoreEchoShot: (playerId: number, inputSeq: number) => boolean;
    };
    client.id = 4;
    client.predictedShotSeqs = new Set([19]);
    // This remains valid even when transport delivery takes longer than the
    // retired 220 ms wall-clock heuristic.
    expect(client.shouldIgnoreEchoShot(4, 19)).toBe(true);
    expect(client.shouldIgnoreEchoShot(4, 19)).toBe(false);
    expect(client.shouldIgnoreEchoShot(5, 19)).toBe(false);
  });
  ...
  it.each([50, 100, 200])('%ims RTT two-client walk agrees after jittered delivery', async (rttMs) => {
    ...
    sock.push({
      v: 3, t: 'welcome', id: player.id, seed: 'rtt-walk', genVersion: ARENA_GEN_VERSION,
      gridHash: arenaGridHash(sim.map.grid, sim.map.pickups), tick: sim.tick,
      snapshot: sim.snapshot(),
    });
    const rivalWelcome = rivalClient.connect('ws://x/arena', 'B');
    rivalSock.push({
      v: 3, t: 'welcome', id: rival.id, seed: 'rtt-walk', genVersion: ARENA_GEN_VERSION,
      gridHash: arenaGridHash(sim.map.grid, sim.map.pickups), tick: sim.tick,
      snapshot: sim.snapshot(),
    });
    await welcomeP;
